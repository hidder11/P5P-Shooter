...
VoxelLandscape.prototype.update = function() {
    ...
    while (true) {
        ...
        if (box) {
            ...
            for (var i = xi; i < nx + 1; i++) {
                if (!this.isFilled(i, yi, zi) || (this.isBoxified(i, yi, zi) &&
                        this.getBoxIndex(i, yi, zi) !== -1)) {
            ...
            for (var i = xi; !found && i < xi + box.nx; i++) {
                for (var j = yi; !found && j < ny + 1; j++) {
                    if (!this.isFilled(i, j, zi) ||
                        (this.isBoxified(i, j, zi) &&
                            this.getBoxIndex(i, j, zi) !== -1)) {
                        // Can't merge this box. Make sure we limit the mergeing
                ...
                for (var j = yi; !found && j < yi + box.ny; j++) {
                    for (var k = zi; k < nz + 1; k++) {
                        if (!this.isFilled(i, j, k) ||
                            (this.isBoxified(i, j, k) &&
                                this.getBoxIndex(i, j, k) !== -1)) {
                            // Can't merge this box. Make sure we limit the mergeing
    ...
    for (var i = 0; i < this.boxes.length; i++) {
        ...
        // Replace box shapes
        b.addShape(new CANNON.Box(
            new CANNON.Vec3(b.nx * sx * 0.5, b.ny * sy * 0.5, b.nz * sz *
                0.5)));
